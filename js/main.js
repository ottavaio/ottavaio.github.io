...
function initFooterResponsive() {
  ...
  var toggleFooter = function (evt) {
    ...
    var currentTitle = document.querySelector(
      "#footer-upper .sections .footer-open-section"
    );
    var currentList = document.querySelector(
      "#footer-upper .sections .footer-show-list"
    );
...
function browserDemoDetection() {
  ...
  if (
    isIOS ||
    (!isIOS && isMobile && (screenMinLength < 640 || mightNotBeChrome))
  ) {
...
window.addEventListener('load', function () {
  ...
  copyright.insertBefore(year, copyright.lastElementChild);
});
